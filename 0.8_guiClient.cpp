...
{
	gtk_init(&argc, &argv);
	GtkWidget *window, *hbox, *button, *vbox, *spacer,
...
/* Sliders */
	...
	tempbox = make_sliderBox((char* )"Arm extension", (char *)"cm", 0, 0, 80);
	gtk_box_pack_start(GTK_BOX(vbox), tempbox, false, false, 0);

	// Rotation
	
	tempbox = make_sliderBox((char* )"Arm rotation", (char *)"degrees", 0, -30, 30);
	gtk_box_pack_start(GTK_BOX(vbox), tempbox, false, false, 0);
	...
	tempbox = make_sliderBox((char* )"Horizontal position", (char *)"cm", 0, 0, 100); // TODO find out max of this
	gtk_box_pack_start(GTK_BOX(vbox), tempbox, false, false, 0); 
...
/* end of sliders */
	...
	gtk_window_set_title(GTK_WINDOW(window), "JMP Client software, v0.8");
